...
def main():
    ...
    pynet.horizontal_rule( "Current rtr1 config" )
    print rtr1.send_command('show run | inc logging')
    pynet.horizontal_rule("Entering config mode and applying config in ./excercise8-config" )
    rtr1.config_mode()
    rtr1.send_config_from_file(config_file='excercise8-config')
    pynet.horizontal_rule( "New rtr1 config" )
    rtr1.exit_config_mode()
    print rtr1.send_command('show run | inc logging')
    pynet.horizontal_rule( "Current rtr2 config" )
    print rtr2.send_command('show run | inc logging')
    pynet.horizontal_rule("Entering config mode and applying config in ./excercise8-config" )
    rtr2.config_mode()
    rtr2.send_config_from_file(config_file='excercise8-config')
    pynet.horizontal_rule( "New rtr2 config" )
